...
from ezaero.vlm.steady import Simulation, WingParameters, MeshParameters, FlightConditions

INFINITE_WING = {
    "wing": WingParameters(root_chord=1.0, tip_chord=1.0, planform_wingspan=10000, sweep_angle=0.0, dihedral_angle=0.0),
...
def rectangular_wing_simulation():
    ...
    sim = Simulation(wing_parameters=wing_params, mesh_parameters=mesh_params, flight_conditions=flight_cond)
    return sim
...
def test_wing_panels_shape(rectangular_wing_simulation):
    ...
    assert rectangular_wing_simulation.wing_panels.shape == (rectangular_wing_simulation.mesh.m, rectangular_wing_simulation.mesh.n, 4, 3)

def test_wing_vortex_panels_shape(rectangular_wing_simulation):
    rectangular_wing_simulation._build_wing_vortex_panels()
    assert rectangular_wing_simulation.vortex_panels.shape == (rectangular_wing_simulation.mesh.m, rectangular_wing_simulation.mesh.n, 4, 3)
...
def test_panel_normal_vectors_shape(rectangular_wing_simulation):
    ...
    assert rectangular_wing_simulation.normals.shape == (rectangular_wing_simulation.mesh.m, rectangular_wing_simulation.mesh.n, 3)
...
def test_calculate_wing_planform_surface(rectangular_wing_simulation):
    ...
    assert rectangular_wing_simulation.panel_surfaces.shape == (rectangular_wing_simulation.mesh.m, rectangular_wing_simulation.mesh.n)

def test_total_planform_surface(rectangular_wing_simulation):
    expected_rectangular_surface = rectangular_wing_simulation.wing.root_chord * rectangular_wing_simulation.wing.planform_wingspan
    calculated_rectangular_surface = rectangular_wing_simulation.panel_surfaces.sum()
    np.testing.assert_almost_equal(calculated_rectangular_surface, expected_rectangular_surface)

def test_wake_shape(rectangular_wing_simulation):
    rectangular_wing_simulation._build_wake()
    assert rectangular_wing_simulation.wake.shape == (rectangular_wing_simulation.mesh.n, 4, 3)
...
def test_panel_circulation_shape(rectangular_wing_simulation):
    ...
    rectangular_wing_simulation.net_circulation.shape = (rectangular_wing_simulation.mesh.m, rectangular_wing_simulation.mesh.n)

def test_run(rectangular_wing_simulation):
    rectangular_wing_simulation.run()

@pytest.mark.parametrize("angle_of_attack", np.array((-2, -1, 0, 1, 2, 5)) * np.pi / 180)
def test_cl_for_infinite_wing(angle_of_attack):
    flcond = FlightConditions(ui=50.0, aoa=angle_of_attack, rho=1.0)
    sim = Simulation(
        wing_parameters=INFINITE_WING["wing"], mesh_parameters=INFINITE_WING["mesh"], flight_conditions=flcond
    )
    res = sim.run()
...
def test_cl_slope_vs_aspect_ratio_for_slender_wing():
    ...
    clas = []
    for bp in bps:
        wing = WingParameters(root_chord=1.0, tip_chord=1.0, planform_wingspan=bp, sweep_angle=0.0, dihedral_angle=0.0)
        res = Simulation(wing_parameters=wing, mesh_parameters=mesh, flight_conditions=flcond).run()
